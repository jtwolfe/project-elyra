...
def test_record_and_status_block_fractions() -> None:
    ...
    assert snap["meal_budget_tokens"] == 50_000
    assert snap["model_window_tokens"] == 500_000
    assert abs(snap["meal_used_fraction"] - (12_000 / 50_000)) < 1e-9
...
def test_status_block_overrides_budget_from_caller() -> None:
    context_meter.record_meal(1000, meal_budget_tokens=50_000, model_window_tokens=500_000)
    snap = context_meter.status_block(
