...
transition to ready; optional close-without-review counter.
...
class GoalsStore:
    ...
    def update_goal(
        ...
        *,
        ...
        result: dict[str, Any] = {"ok": True, "goal": dict(goal)}
    ...
    def create_task(
        ...
        *,
        ...
        if status == "ready":
            self._fire_task_ready(task["id"], goal_id)
        return dict(task)
    ...
    def update_task(
        ...
        *,
        ...
        does not re-fire. Dedupe is the hook's contract.
        """
        doc = self._load()
        found = self._find_task(doc, task_id)
        if found is None:
        ...
        if became_ready:
            self._fire_task_ready(task["id"], task["goal_id"])
