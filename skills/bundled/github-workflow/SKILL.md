...
1. **Orient** the job: goal/task acceptance, base branch, whether a worktree is warranted, L/M/H if self-mod.
...
6. **Ship path:** `git_add` → `git_commit` on the feature branch; `gh_pr_create` / `gh_pr_list` / `gh_pr_view` when remote review is needed; `gh_issue_*` / `gh_api` as escape hatch only when structured tools lack the op. **No auto-merge.**
