...
| [design-conversation-list-and-group-tools.md](glass/design-conversation-list-and-group-tools.md) | **Draft** | Conversation list discovery refresh + `create_group` / `update_group` tools (v1.1); stack on **`feature/elyra-conv-tool`** |
...
| [design-mm-embed-buildout.md](memory/design-mm-embed-buildout.md) | **Shipped (code; dogfood pending)** | Multimodal semantic loop complete in code (PR0–PR7 #124); on `working`/`main`; operator checklist [mm-embed-dogfood.md](../state/memory/mm-embed-dogfood.md); **not** Gate B default-on |
| [design-view-media-meal-rebalance.md](memory/design-view-media-meal-rebalance.md) | **Shipped (code; dogfood pending)** | Host `view_media` + moment viewing set + expand(wake ∪ viewing) + outer meal rebalance; on `working`/`main`; checklist [mm-view-dogfood.md](../state/memory/mm-view-dogfood.md) |
...
| [design-memory-edges-and-traversal.md](memory/design-memory-edges-and-traversal.md) | **Shipped (code; dogfood partial)** | Durable edges + pure semantic start + raised budgets (#98/#120/#103/#105); on `working`/`main`; checklist [edges-traversal-dogfood.md](../state/memory/edges-traversal-dogfood.md); **not** Gate B / `durable_edges_enabled` default-on; #61 free-browse PR8 landed; polish2 residuals [#125](https://github.com/jtwolfe/project-elyra/issues/125) |
| [design-memory-edges-polish1.md](memory/design-memory-edges-polish1.md) | **Shipped (code; dogfood partial)** | Edge enrichment polish1: unified semantic wait + deferred recalls + host ~d2.5 local_map + walk maneuvers + dev edge backfill + glass sticky; landed `working`/`main` @ `161a820`; checklist [edges-traversal-dogfood.md](../state/memory/edges-traversal-dogfood.md); **not** Gate B; residuals [#125](https://github.com/jtwolfe/project-elyra/issues/125) polish2 |
