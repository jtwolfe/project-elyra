...
| **Work product (when executed)** | Isolated report package under `docs/investigations/lance-debug1/` only |
...
3. **Doc package layout** under `docs/investigations/lance-debug1/` with named artifacts, templates, and “what each must prove.”
...
- Changing default product docs under `docs/stretch-2/**` beyond optional one-way cross-links **from** `docs/investigations/lance-debug1/`.
...
- All inspection artifacts live under **`docs/investigations/lance-debug1/`**.
- Scripts are **hermetic helpers** colocated with that package (`docs/investigations/lance-debug1/scripts/`) so they never look like product entrypoints and are easy to drop after the investigation.
...
### Folder tree for `docs/investigations/lance-debug1/`
...
1. Inspection-only; work product is `docs/investigations/lance-debug1/**`.
2. Scripts colocated under `docs/investigations/lance-debug1/scripts/` (not top-level `scripts/lance-debug1/`).
...
Each procedure is a markdown file under `docs/investigations/lance-debug1/procedures/` with: purpose, prerequisites, safety class, exact commands, expected observations, evidence capture path, pass/fail criteria for hypotheses.
...
**Script:** `docs/investigations/lance-debug1/scripts/api_matrix.py`
...
3. **No patched product `_load`.** Optional script `docs/investigations/lance-debug1/scripts/consumer_compare.py` (PR4):
...
python docs/investigations/lance-debug1/scripts/api_matrix.py \
  --uri "$LANCE_DEBUG_URI" \
  --out docs/investigations/lance-debug1/evidence/$(date +%Y-%m-%d)-run-01/api-matrix.json

# Later W1 (PR3): load_parity requires marker at $QROOT/.lance-debug1-quarantine
# python docs/investigations/lance-debug1/scripts/load_parity.py --data-dir "$LANCE_DEBUG_DATA_DIR" ...
```
...
- Tiny synthetic multi-version table under `docs/investigations/lance-debug1/scripts/fixtures/` **for tests only** — optional PR2.
...
|-------|---------|-------------|
| PR1 | Scaffold `docs/investigations/lance-debug1/` tree + SAFETY + templates + caller_grep | Docs only |
...
The inspection is complete when `docs/investigations/lance-debug1/BUG-DOSSIER.md` can state all of the following with evidence links:
...
| Q7 | Path for scripts: `docs/investigations/lance-debug1/scripts/` vs `scripts/lance-debug1/`? | **docs colocated** (KD2) | — |
...
| **KD2** | All work product under `docs/investigations/lance-debug1/`; scripts colocated in `docs/investigations/lance-debug1/scripts/` | Isolation from stretch-2 product docs and top-level product scripts; easy to archive |
...
Inspection-only, incremental, independently mergeable. **No production memory behavior changes.** All script paths under `docs/investigations/lance-debug1/scripts/`.

### PR1 — Scaffold `docs/investigations/lance-debug1/` package

| Field | Value |
|-------|--------|
| **Title** | docs(lance-debug1): scaffold inspection package layout and safety rules |
| **Files** | `docs/investigations/lance-debug1/README.md`, `SAFETY.md` (idle copy, W1 open, deny-list, dual-connect), `OBSERVED-FACTS.md` (snapshot-labeled facts + H1 mechanism candidate), `HYPOTHESES.md` (H1/H1a/H1b–H12 untested), `FAULT-BUCKETS.md`, `CODE-PATH-MAP.md` (fresh grep line pins), `TO-ARROW-CALLERS.md`, `docs/investigations/lance-debug1/scripts/caller_grep_report.py` (optional helper) + `scripts/README.md`, empty `procedures/` stubs, `evidence/_template-*.md` |
| **Dependencies** | None |
...
| **Files** | `docs/investigations/lance-debug1/scripts/env_check.py`, `docs/investigations/lance-debug1/scripts/api_matrix.py`, `docs/investigations/lance-debug1/scripts/quarantine_copy.sh`, `docs/investigations/lance-debug1/procedures/P01-offline-api-matrix.md`, `docs/investigations/lance-debug1/REPRO-RECIPES.md` (R1), optional `tests/test_lance_debug1_api_matrix_fixture.py` + `docs/investigations/lance-debug1/scripts/fixtures/` |
| **Dependencies** | PR1 |
...
| **Files** | `docs/investigations/lance-debug1/scripts/load_parity.py`, `docs/investigations/lance-debug1/scripts/version_sample.py`, `docs/investigations/lance-debug1/procedures/P02-*.md`, `P08-*.md`, `docs/investigations/lance-debug1/VERSION-ARCHAEOLOGY.md`, `docs/investigations/lance-debug1/API-COMPARISON.md` template |
| **Dependencies** | PR2 |
...
| **Files** | `docs/investigations/lance-debug1/procedures/P03–P07,P09`, `docs/investigations/lance-debug1/adjacency/*.md`, `docs/investigations/lance-debug1/EVIDENCE-MATRIX.md`, optional `docs/investigations/lance-debug1/scripts/consumer_compare.py`, glass curl notes |
...
| **Files** | `docs/investigations/lance-debug1/evidence/YYYY-MM-DD-run-NN/**` (counts/ids), filled comparison docs, `HYPOTHESES.md` statuses, `BUG-DOSSIER.md`, `README.md` status → Complete |
