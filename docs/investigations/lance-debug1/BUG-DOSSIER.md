...
**Package:** `docs/investigations/lance-debug1`  
...
./docs/investigations/lance-debug1/scripts/quarantine_copy.sh /path/to/data/memory /tmp/lance-q-YYYYMMDD
export LANCE_DEBUG_DATA_DIR=/tmp/lance-q-YYYYMMDD/data
export LANCE_DEBUG_URI=$LANCE_DEBUG_DATA_DIR/memory/lance
RUN=docs/investigations/lance-debug1/evidence/$(date +%Y-%m-%d)-run-NN
mkdir -p "$RUN"

$PY docs/investigations/lance-debug1/scripts/api_matrix.py --uri "$LANCE_DEBUG_URI" --out "$RUN/api-matrix.json"
# Expect: n_arrow ≈ 10; n_full large; h1a.ok; h1b.path=head_n_full

$PY docs/investigations/lance-debug1/scripts/load_parity.py \
