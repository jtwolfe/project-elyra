# Lance debug package 1 (`docs/investigations/lance-debug1`)

> **Status: Archive / INVESTIGATION** — sealed forensic bag; do not rewrite `evidence/**`.  
> Product fix landed outside this package. Index: [../README.md](../README.md). Prefer code on `working`.

**Inspection complete** — dogfood evidence sealed (`2026-07-29-run-01`) + [BUG-DOSSIER.md](BUG-DOSSIER.md). **Product fix landed** in `elyra/memory` (`fcb5130`; design [design-fix-load-truncation.md](design-fix-load-truncation.md)). This package remains historical evidence; do not rewrite the sealed bag.

| Field | Value |
|-------|--------|
| **Class** | INVESTIGATION |
| **Status** | Archive (inspection complete; product fix outside bag) |
| **Audience** | Archaeology / operators replaying evidence |
| **Normative?** | No — sealed bag law (KD12) |
| **Intent** | Inspection and fault isolation (historical) — product fix is **outside** this package |
| **Isolation** | Evidence bag under `docs/investigations/lance-debug1/`; product code in `elyra/memory/**` |
...
| **Moved from** | `docs/lance-debug1/` (docs reorg PR5 / #121) |
...
./docs/investigations/lance-debug1/scripts/quarantine_copy.sh data/memory /tmp/lance-q-YYYYMMDD
